    ...
    def __init__(self, response):
        ...
        try:
            ...
            if "code" in json_res.keys() and "msg" in json_res.keys():
                ...
                if (json_res['code'] != "0") and 'data' in json_res.keys() and (isinstance(json_res['data'], list) and len(json_res['data']) > 0 and 'sCode' in json_res['data'][0].keys()):  # failed operation, need to look inside 'data' or 'msg' to see more information
                    self.code = json_res['data'][0]['sCode']
                    self.message = json_res['data'][0]['sMsg']
                else:
